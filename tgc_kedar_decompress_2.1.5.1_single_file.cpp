...
#define phraseNewCharSizeDir "bv3_"
...
int main(int argc, char** argv)
{
	...
	if(fast_decomp)
	{
		short temp = 0;
		unsigned char* temp_char = (unsigned char*)&temp;
		ofstream output_file("output_" + file_names[file_id], ios::binary);
		...
		for(int j = 0 ; j < number_of_blocks ; j++ )
		{
			...
			for(int i = 0 ; i < block_data[j].size() ; i++ )
			{
				...
				//cout << (unsigned int)temp_char[0] << " " << (unsigned int)temp_char[1] << " ";
				 //if it is the last instance then remove the last byte appended
				//if(file_size_byte%2 == 1 && i==block_data[j].size()-1 && j==number_of_blocks-1 )
				//	output_file << (unsigned char)temp_char[0];
				//else
	...
	{	
		short temp = 0;
		ofstream output_file("output1_" + file_names[file_id], ios::binary);
		unsigned char* temp_char = (unsigned char*)&temp;
