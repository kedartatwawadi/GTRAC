/***********************************************************************************************/
/*
/*	Output the bv vectors to a single file etc.	
/* 
/***********************************************************************************************/
...
#define phraseNewCharSizeDir "bv3"
#define logFolderDir "log_folder_5.1/log"
...
void compress(void);
void parse_file(unsigned short *d);
void store_start(string &name);

inline unsigned long hash_fun(unsigned short *p, int pos, int ver);
void insert_into_ht(file_id_t file_id, unsigned short *p, int ver);
inline pair<int, int> find_match(unsigned short *p, int pos, int ver);
...
void createNewCharSizeBitVector(vector<bool> phrase_newchar_size, int file_id )
{
	bvb.Clear();	
	for( int j = 0;j < phrase_newchar_size.size(); j++)
		bvb.PushBack( phrase_newchar_size[j] );
	bvb.Build(phraseNewCharSize[file_id]);
}
...
void prepare_ht(void)
{
	// set the appropriate number of hashtable slots
...
unsigned short* read_file(string &name)
{
	...
	if( file_size_byte%2 == 1)
	{
		fread(d, 2, file_size-1, in); // The last one is the padding bit
		d[file_size-1] = 0; // Needs fixing
	}
	else
	{
		fread(d, 2, file_size, in);	
	}
...
bool prepare_files(string output_name)
{
	...
	phraseNewCharSize = new RSDic[no_files];
...
// ***************************************************************
void parse_file(unsigned short* d, int file_id)
{
	cout << "Parsing file id: " << file_id << endl;
	string filename = file_names[file_id];
	ofstream out_file((logFolderDir + filename + ".txt"), ios::out | ios::binary | ios::trunc );
	...
	vector<bool> phrase_newchar_size;
	...
	//	cout << match.first << ", " << match.second << endl;
		if(match.second < HASH_LEN1 + HASH_STEP1 - 1)
			...
			if(match.first >= 0)
			{
				//match.first = (unsigned short) file_id - match.first;
				if( match.first > 255)
				{
					unsigned short match_file_id = (unsigned short) match.first;
					out_file.write( (char*)&match_file_id, sizeof( unsigned short ) );
					phrase_source_size.push_back(false);
				}
				else
				{
					unsigned char match_file_id = (unsigned char) match.first;
					out_file.write( (char*)&match_file_id, sizeof( unsigned char ) );
			...
			// WRITE NEWCHAR
			/******************************/
			unsigned short new_char = d[pos]; 
			if( new_char > 255)
			{
				out_file.write( (char*)&new_char, sizeof( unsigned short ) );
				phrase_newchar_size.push_back(false);
			}
			else
			{
				unsigned char new_char_byte = (unsigned char) new_char;
				out_file.write( (char*)&new_char_byte, sizeof( unsigned char ) );
				phrase_newchar_size.push_back(true);
			}
			// out_file.write( (char*)&d[pos], sizeof( unsigned short ) );
	...
	createNewCharSizeBitVector(phrase_newchar_size,file_id);
	out_file.close();
}
...
void compress(void)
{
	unsigned short *d; // represents the data file
	cur_id_file = 0;
	for(int i = 0; i < file_names.size(); ++i) // iterating over files
	{
		...
		if((d = read_file(file_names[cur_id_file])) == NULL)
			continue;

		// Store d in the data vector (used to find the match)
		data.push_back(d);
...
void output_all_succint_bv_files()
{
	...
	output_bv_files(phraseNewCharSize, phraseNewCharSizeDir);
}
...
bool check_data(int argc, char *argv[])
{
	...
	fseek(in, 0, SEEK_END);
	file_size_byte = (int) ftell(in);
	// This is because now we are dealing with shorts.
	file_size = file_size_byte/2 + file_size_byte%2; //to consider the case where we have odd number of bytes
	cout << "The file size in terms of unsigned shorts is: "<< file_size << endl;
	cout << "The file size in bytes is: " << file_size_byte << endl; 
	fclose(in);
